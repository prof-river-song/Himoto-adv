...
def getRace(self=None):
        ...
        race = ""
        while (race.lower() not in races):
            if race != "":
                print("Invalid")
            race=input(prompt)
        # This is a better way of writing the above example
...
def get_choice2(player_type, choice):
     print(f"Debugging Choice: {choice}")
     # global choice2
     if choice == 1:
         ans = int(input(player_type.choices["part3_choice"]))
         if ans == 1:
             print(player_type.choices["part3_end_one"])
         elif ans ==2 :
             print(player_type.choices["part3_end_two"])
         else: print(player_type.choices["game_over"])
     elif choice == 2:
         ans = int(input(player_type.choices["part4_choice"]))
         if ans == 1:
             choice = 1
...
get_choice2(game_state.player, game_state.last_choice)
